...
def run_qopher(config):
    ...
    for icand, candidate in enumerate(candidates):
        ...
        if config.plot:
            ...
            slope, intercept, r_value, p_value, std_err = stats.linregress(
                f[indx], ratio[indx])
            print slope, 'b', 'rvalue', r_value, 'pvalue', p_value
            ...
            print '>0, <0:',  slope_positive, slope_negative, counter
    ...
    num.savetxt(pjoin(config.outdir, '%s/qs_inv.txt' % (config.whitelist[0][1]), num.array(qs).T))
